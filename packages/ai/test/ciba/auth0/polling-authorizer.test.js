...
describe("Auth0PollingCIBAAuthorizer#authorize", () => {
  describe("constructor", async () => {
    it("should construct with URLs", () => {
      ...
      expect(authorizer.authorizationURL).toBe(
        "http://example.test/bc-authorize"
      );
    ...
    it("should construct with domain", () => {
      ...
      expect(authorizer.authorizationURL).toBe(
        "https://example.auth0.com/bc-authorize"
      );
  ...
  describe("#authorize", async () => {
    it(
      ...
      async () => {
        ...
        const x = await authorizer.authorize({
          loginHint: "auth0|1234567890",
          scope: ["openid"],
        });
        ...
        expect(x.accessToken.value).toEqual("G5kXH2wHvUra0sHlDy1iTkDJgsgUO1bN");
      }
    ); // should request authorization with login hint formatted as subject identifier
  }); // #authorize
});
